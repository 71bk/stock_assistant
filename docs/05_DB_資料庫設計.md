# 資料庫設計

> 版本：v1.1（對應 `stock_project_v1_1_ddl.sql`）  
> 資料庫：PostgreSQL 15+ with pgvector

---

## 命名規範

- **Schema**：`app`（業務資料）、`vector`（向量資料）
- **表名**：snake_case，名詞複數（如 `users`、`stock_trades`）
- **欄位名**：snake_case，明確語意
- **時間**：一律使用 `TIMESTAMPTZ`，存儲 **UTC**
- **日期**：使用 `DATE`，存儲 **西元日期**（民國日期在匯入時轉換）
- **金額/數量**：`NUMERIC(20, 6)` 或 `NUMERIC(20, 8)`（避免浮點誤差）
- **狀態/類型**：使用 `TEXT` + `CHECK` 約束，存儲字串 code（不用 enum ordinal）
- **主鍵**：`BIGINT GENERATED ALWAYS AS IDENTITY`
- **外鍵**：使用 `ON DELETE CASCADE` 或 `ON DELETE RESTRICT`（依資料關聯性）
- **審計欄位**：`created_at`、`updated_at`（使用 trigger 自動更新）

---

## ERD / Table 概覽

```mermaid
erDiagram
    users ||--o{ portfolios : has
    users ||--|| user_settings : has
    portfolios ||--o{ accounts : contains
    portfolios ||--o{ stock_trades : contains
    portfolios ||--o{ user_positions : contains
    
    markets ||--o{ exchanges : contains
    markets ||--o{ instruments : listed_in
    exchanges ||--o{ instruments : traded_on
    instruments ||--o{ instrument_aliases : has
    instruments ||--o| etf_profiles : "may have (if ETF)"
    instruments ||--o| warrant_profiles : "may have (if WARRANT)"
    
    instruments ||--o{ stock_trades : references
    instruments ||--o{ user_positions : references
    instruments ||--o{ prices : has
    instruments ||--o{ corporate_actions : has
    
    users ||--o{ files : uploads
    users ||--o{ statements : imports
    statements ||--o{ statement_trades : contains
    statements ||--o{ statement_holdings : contains
    
    users ||--o{ ai_reports : creates
    users ||--o{ rag_documents : owns
    users ||--o{ conversations : owns
    users ||--o{ analytics_events : generates
    conversations ||--o{ conversation_messages : contains
    rag_documents ||--o{ rag_chunks : split_into
```

---

## 核心表（users / trades / positions）

### app.users
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| email | TEXT UNIQUE | 電子郵件 |
| google_sub | TEXT UNIQUE | Google OAuth subject ID |
| display_name | TEXT | 顯示名稱 |
| picture_url | TEXT | Google 頭像 URL |
| status | TEXT | 狀態：ACTIVE/SUSPENDED/PENDING |
| last_login_at | TIMESTAMPTZ | 最後登入時間 |
| created_at | TIMESTAMPTZ | 建立時間 |
| updated_at | TIMESTAMPTZ | 更新時間 |

### app.user_settings
| 欄位 | 類型 | 說明 |
|------|------|------|
| user_id | BIGINT PK FK | 使用者 ID |
| base_currency | CHAR(3) | 基準幣別（預設 TWD） |
| display_timezone | TEXT | 顯示時區（預設 Asia/Taipei） |

### app.analytics_events
| 欄位 | 類型 | 說明 |
|------|------|------|
| event_id | UUID PK | 前端產生的冪等事件 ID |
| user_id | BIGINT FK | JWT 驗證後的使用者 ID |
| session_id | UUID | 瀏覽器分頁工作階段 |
| event_type | TEXT | 目前支援 `PAGE_VIEW` |
| route | TEXT | SPA pathname，不含 query string |
| occurred_at | TIMESTAMPTZ | 事件發生時間 |
| created_at | TIMESTAMPTZ | 伺服器寫入時間 |

- 不保存 Email、IP、User-Agent、URL query 或投資／AI 內容。
- 原始事件預設保留 90 天，由每日排程清理。

### app.portfolios
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID |
| name | TEXT | 投資組合名稱 |
| base_currency | CHAR(3) | 基準幣別 |

### app.stock_trades
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID |
| portfolio_id | BIGINT FK | 投資組合 ID |
| account_id | BIGINT FK | 券商帳戶 ID（可選） |
| instrument_id | BIGINT FK | 商品 ID |
| trade_date | DATE | 交易日期（西元） |
| settlement_date | DATE | 交割日期（可選） |
| side | TEXT | 買賣方向：BUY/SELL |
| quantity | NUMERIC(20,6) | 數量 |
| price | NUMERIC(20,8) | 單價 |
| currency | CHAR(3) | 幣別 |
| gross_amount | NUMERIC(20,6) | 交易總額 |
| fee | NUMERIC(20,6) | 手續費 |
| tax | NUMERIC(20,6) | 稅金 |
| net_amount | NUMERIC(20,6) | 淨額 |
| source | TEXT | 來源：OCR/MANUAL/API/MIGRATION |
| source_ref_id | BIGINT | 來源參照 ID |
| row_hash | CHAR(64) UNIQUE | SHA256 去重雜湊 |

### app.user_positions
| 欄位 | 類型 | 說明 |
|------|------|------|
| portfolio_id | BIGINT PK FK | 投資組合 ID |
| instrument_id | BIGINT PK FK | 商品 ID |
| total_quantity | NUMERIC(20,6) | 總持股數量 |
| avg_cost_native | NUMERIC(20,8) | 加權平均成本 |
| currency | CHAR(3) | 成本幣別 |
| updated_at | TIMESTAMPTZ | 更新時間 |

> `user_positions` 為衍生快照（由 `stock_trades` 回算）。  
> 若發生異常可透過 `POST /api/admin/portfolios/positions-rebuild` 手動修復。

---

## 多市場主檔（markets / instruments）

### app.markets
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| code | TEXT UNIQUE | 市場代碼：TW/US |
| name | TEXT | 市場名稱 |
| timezone | TEXT | 市場時區 |
| default_currency | CHAR(3) | 預設幣別 |

### app.exchanges
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| market_id | BIGINT FK | 市場 ID |
| mic | TEXT | 交易所代碼（MIC） |
| code | TEXT | 簡稱（可選） |
| name | TEXT | 交易所名稱 |

### app.instruments
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK IDENTITY | 主鍵（自增序列，對外 API 轉字串） |
| market_id | BIGINT FK | 市場 ID |
| exchange_id | BIGINT FK | 交易所 ID |
| ticker | TEXT | 股票代號（如 AAPL、2330、00926） |
| symbol_key | TEXT UNIQUE | 唯一識別碼（如 US:XNAS:AAPL、TW:XTAI:00926） |
| name_zh | TEXT | 中文名稱 |
| name_en | TEXT | 英文名稱 |
| currency | CHAR(3) | 交易幣別 |
| asset_type | TEXT | 資產類型：STOCK/ETF/WARRANT（預設 STOCK） |
| status | TEXT | 狀態：ACTIVE/DELISTED/SUSPENDED |

> **v1.2 新增**：`asset_type` 欄位用於區分股票與 ETF，ETF 可額外關聯 `etf_profiles` 表。  
> **v1.4 新增**：`asset_type` 支援 WARRANT，並新增 `warrant_profiles` 表。  
> **設計說明**：`id` 使用 BIGINT 自增而非 UUID，效能更優且易讀；`symbol_key` 作為跨市場唯一識別碼。
> **同步策略（現況）**：已提供管理端手動同步（equity / warrants），可再擴充定期同步排程。

### app.instrument_aliases
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| instrument_id | BIGINT FK | 商品 ID |
| source | TEXT | 別名來源：OCR/MANUAL/DATA_VENDOR |
| alias_ticker | TEXT | 別名（如 BRK-B 對應 BRK.B） |

### app.etf_profiles
| 欄位 | 類型 | 說明 |
|------|------|------|
| instrument_id | BIGINT PK FK | 商品 ID（僅 ETF） |
| underlying_type | TEXT | 標的類型：INDEX/THEMATIC/FACTOR/ACTIVE/OTHER |
| underlying_name | TEXT | 標的名稱（如「S&P 500 Index」、「全球AI×智能電動車」） |
| as_of_date | DATE | 資料更新日期（可選） |
| updated_at | TIMESTAMPTZ | 更新時間 |

> **v1.2 新增**：ETF 專屬資訊表，用於儲存 ETF 的追蹤標的資訊（不依賴行情 API）。  
> **FK 約束**：`instrument_id` 參照 `instruments(id)` ON DELETE CASCADE

### app.warrant_profiles
| 欄位 | 類型 | 說明 |
|------|------|------|
| instrument_id | BIGINT PK FK | 商品 ID（僅 WARRANT） |
| underlying_symbol | TEXT | 標的股代號 |
| expiry_date | DATE | 到期日 |
| created_at | TIMESTAMPTZ | 建立時間 |
| updated_at | TIMESTAMPTZ | 更新時間 |

> **v1.4 新增**：權證專屬資訊表，用於儲存標的股與到期日（名冊同步後寫入）。  
> **FK 約束**：`instrument_id` 參照 `instruments(id)` ON DELETE CASCADE

---

## 行情與匯率（prices / fx_rates）

### app.prices
| 欄位 | 類型 | 說明 |
|------|------|------|
| instrument_id | BIGINT PK FK | 商品 ID |
| time_interval | TEXT PK | 時間週期：1m/5m/15m/1h/1d/1w/1mo |
| ts_utc | TIMESTAMPTZ PK | 時間戳（UTC） |
| open | NUMERIC(20,8) | 開盤價 |
| high | NUMERIC(20,8) | 最高價 |
| low | NUMERIC(20,8) | 最低價 |
| close | NUMERIC(20,8) | 收盤價 |
| volume | NUMERIC(24,6) | 成交量 |
| source | TEXT | 資料來源（yahoo/fugle/...） |

### app.fx_rates
| 欄位 | 類型 | 說明 |
|------|------|------|
| base_currency | CHAR(3) PK | 基準幣別 |
| quote_currency | CHAR(3) PK | 報價幣別 |
| ts_utc | TIMESTAMPTZ PK | 時間戳（UTC） |
| rate | NUMERIC(20,8) | 匯率 |

---

## OCR staging（jobs / statements）

### app.files
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID |
| provider | TEXT | 儲存供應商：local/s3/minio |
| bucket | TEXT | 儲存桶（可選） |
| object_key | TEXT | 物件鍵 |
| sha256 | CHAR(64) | 檔案雜湊（per-user 去重）|
| size_bytes | BIGINT | 檔案大小 |
| content_type | TEXT | MIME 類型 |

> **唯一性約束（v1.1 V5 Migration）**：`UNIQUE (user_id, sha256)` - 同一使用者不可上傳相同檔案，但不同使用者可以上傳同一檔案。

### app.ocr_jobs
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID |
| file_id | BIGINT FK | 檔案 ID |
| statement_id | BIGINT FK | 對應的 statement ID |
| status | TEXT | 狀態：QUEUED/RUNNING/FAILED/DONE |
| progress | INT | 進度（0-100） |
| error_message | TEXT | 錯誤訊息 |

### app.statements（匯入批次）
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID |
| portfolio_id | BIGINT FK | 投資組合 ID |
| source | TEXT | 來源：OCR/MANUAL |
| file_id | BIGINT FK | 檔案 ID（OCR 才有） |
| status | TEXT | 狀態：DRAFT/REVIEWED/CONFIRMED/FAILED/SUPERSEDED |
| period_start | DATE | 對帳單起始日 |
| period_end | DATE | 對帳單結束日 |
| raw_text | TEXT | OCR 原始文字 |
| parsed_json | JSONB | 解析結果 JSON |
| superseded_at | TIMESTAMPTZ | 被新版本取代的時間（僅 SUPERSEDED 才有） |

### app.statement_trades（交易草稿）
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| statement_id | BIGINT FK | 所屬 statement |
| instrument_id | BIGINT FK | 對應商品（解析後） |
| raw_ticker | TEXT | 原始股票代號 |
| trade_date | DATE | 交易日期 |
| side | TEXT | 買賣方向 |
| quantity | NUMERIC | 數量 |
| price | NUMERIC | 價格 |
| currency | CHAR(3) | 幣別 |
| fee | NUMERIC | 手續費 |
| tax | NUMERIC | 稅金 |
| row_hash | CHAR(64) | 列雜湊（去重） |
| errors_json | JSONB | 錯誤列表 |
| warnings_json | JSONB | 警告列表 |
| **parsed_a_json** | JSONB | **v2 新增：Text LLM 解析結果** |
| **parsed_b_json** | JSONB | **v2 新增：Vision LLM 解析結果** |
| **final_json** | JSONB | **v2 新增：融合後的最終草稿** |
| **diff_json** | JSONB | **v2 新增：A、B 差異清單（欄位級別）** |
| **confidence_a** | NUMERIC | **v2 新增：A 路徑信心分數（0.0-1.0）** |
| **confidence_b** | NUMERIC | **v2 新增：B 路徑信心分數（0.0-1.0）** |
| **merge_rule_version** | TEXT | **v2 新增：融合規則版本（追溯用）** |

> **v2 雙路徑解析**：保留 A（Text LLM）、B（Vision LLM）兩條路徑的解析結果，透過融合層產出 final_json 作為草稿呈現給使用者。diff_json 記錄兩者差異，Review UI 可據此標紅差異欄位並提供「套用 A / 套用 B」選項。

---

## AI/RAG（reports / vectors / chat）

### app.ai_reports
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID |
| instrument_id | BIGINT FK | 相關商品（可選） |
| report_type | TEXT | 報告類型：INSTRUMENT/PORTFOLIO/GENERAL |
| input_summary | JSONB | 輸入摘要 |
| output_text | TEXT | AI 生成內容 |
| created_at | TIMESTAMPTZ | 建立時間 |

### app.conversations
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 對話 ID |
| user_id | BIGINT FK | 使用者 ID |
| title | TEXT | 對話標題（v1 可用首句截斷） |
| prompt_version | TEXT | prompt 版本（便於追溯） |
| prompt_snapshot | TEXT | prompt 快照（便於 debug/回放） |
| summary | TEXT | 摘要（v2 預留） |
| created_at | TIMESTAMPTZ | 建立時間 |
| updated_at | TIMESTAMPTZ | 更新時間 |
| deleted_at | TIMESTAMPTZ | 軟刪（未落地，未來可加） |

### app.conversation_messages
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 訊息 ID |
| conversation_id | BIGINT FK | 對話 ID |
| role | TEXT | system / user / assistant |
| content | TEXT | 訊息內容 |
| status | TEXT | PENDING/COMPLETED/FAILED（可選，用於 SSE） |
| client_message_id | TEXT | 客戶端訊息 ID（可選，用於去重） |
| created_at | TIMESTAMPTZ | 建立時間 |

### vector.rag_documents
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| user_id | BIGINT FK | 使用者 ID（資料隔離） |
| source_type | TEXT | 來源類型：NOTE/REPORT/FILE/WEB |
| source_id | TEXT | 外部來源 ID |
| title | TEXT | 文件標題 |
| meta | JSONB | 元資料 |
| created_at | TIMESTAMPTZ | 建立時間 |

### vector.rag_chunks
| 欄位 | 類型 | 說明 |
|------|------|------|
| id | BIGINT PK | 主鍵 |
| document_id | BIGINT FK | 所屬文件 |
| user_id | BIGINT FK | 使用者 ID（冗餘，提升查詢效率） |
| chunk_index | INT | 區塊索引 |
| content | TEXT | 區塊內容 |
| embedding | vector(1536) | 向量嵌入 |
| meta | JSONB | 元資料 |
| embedding_model | TEXT | Embedding 模型名稱 |
| embedding_version | TEXT | Embedding 版本 |
| dimensions | INT | 向量維度 |
| created_at | TIMESTAMPTZ | 建立時間 |

> **Schema ownership**：`vector.rag_documents`、`vector.rag_chunks` 由 backend Flyway migration 定義，`ai-worker` 依該 schema 進行寫入與查詢，並擁有這兩張表的 row mutation。任何欄位異動都必須同步更新 migration、Java contract test、worker SQL 與相關文件。`ai-worker` 啟動時必須驗證 `vector.rag_chunks.embedding` 的實際 dimension 與 worker 設定一致。

---

## 索引與效能

### 唯一索引
- `users(email)` UNIQUE
- `users(google_sub)` UNIQUE
- `instruments(symbol_key)` UNIQUE
- `instrument_aliases(source, alias_ticker)` UNIQUE
- `prices(instrument_id, time_interval, ts_utc)` PRIMARY KEY
- `fx_rates(base_currency, quote_currency, ts_utc)` PRIMARY KEY
- `stock_trades(row_hash)` UNIQUE
- `files(user_id, sha256)` UNIQUE - v1.1 V5 per-user 唯一性
- `statement_trades(statement_id, row_hash)` UNIQUE
- `rag_chunks(document_id, chunk_index)` UNIQUE
- `conversation_messages(conversation_id, client_message_id)` UNIQUE

### 查詢索引
- `portfolios(user_id)`
- `stock_trades(portfolio_id, trade_date DESC)`
- `stock_trades(instrument_id, trade_date DESC)`
- `user_positions(instrument_id)`
- `files(user_id, created_at DESC)`
- `statements(user_id, created_at DESC)`
- `ai_reports(user_id, created_at DESC)`
- `conversations(user_id, updated_at DESC)`
- `conversation_messages(conversation_id, id)`
- `rag_documents(user_id)`
- `rag_chunks(user_id)` — 用於向量檢索時過濾

### 向量索引
- `rag_chunks(embedding)` USING ivfflat (vector_cosine_ops) WITH (lists = 100)
- 建議資料量大時改用 HNSW 索引

---

## Migration 歷史

### 已執行的 Migrations
| Migration | 功能說明 | 時間點 |
|-----------|---------|--------|
| V1__init.sql | 初始化 Schema（app + vector）、建立所有基礎表 | v1.0 |
| V2__add_constraints_indexes.sql | 加入外鍵約束、索引優化 | v1.0 |
| V3__seed_base_data.sql | 插入基礎資料（markets、exchanges、測試商品） | v1.0 |
| V4__add_market_code_check.sql | 對 markets.code 加入 CHECK 約束（TW/US） | v1.1 |
| V5__files_unique_user_sha256.sql | Files 表加入 (user_id, sha256) 唯一性約束 | v1.1 |
| V6__add_etf_support.sql | 加入 ETF 支援（asset_type + etf_profiles） | v1.2 |
| V13__add_warrant_support.sql | 加入權證支援（asset_type + warrant_profiles） | v1.4 |
| V14__add_conversation_prompt_snapshot.sql | Chat 對話加入 prompt 版本/快照 | v1.4 |

### ETF 支援（V6 詳細說明）
```sql
-- 1. instruments 表加入 asset_type 欄位
ALTER TABLE app.instruments 
ADD COLUMN asset_type TEXT DEFAULT 'STOCK' NOT NULL
CHECK (asset_type IN ('STOCK', 'ETF', 'WARRANT'));

-- 2. 建立 etf_profiles 表
CREATE TABLE app.etf_profiles (
    instrument_id BIGINT PRIMARY KEY,
    underlying_type TEXT NOT NULL CHECK (...),
    underlying_name TEXT NOT NULL,
    as_of_date DATE,
    updated_at TIMESTAMPTZ,
    FOREIGN KEY (instrument_id) REFERENCES app.instruments(id)
);
```

---

## Migration 規則

1. **Migration 工具**：使用 Flyway 管理 DDL 版本
2. **檔案命名**：`V{version}__{description}.sql`（如 `V1__init_schema.sql`）
3. **向下相容**：避免刪除欄位，優先使用 `ADD COLUMN IF NOT EXISTS`
4. **資料遷移**：大型資料遷移應分批執行，避免長時間鎖表
5. **索引建立**：使用 `CONCURRENTLY` 避免阻塞生產環境
6. **時間格式**：所有時間轉換為 UTC 後再入庫
7. **民國日期**：匯入層負責轉換，DB 只存西元日期

---

## 資料流落表

### 交易匯入流程（v2 雙路徑）
```
Upload → files
    ↓
OCR Job → ocr_jobs (QUEUED → RUNNING)
    ↓
    ├─→ A 路徑：OCR Extract Text → Text LLM → parsed_a_json
    └─→ B 路徑：Vision LLM (image/pdf) → parsed_b_json
    ↓
Merge Layer → 對齊 A+B → final_json + diff_json + confidence
    ↓
statements (DRAFT) + statement_trades
    ↓
Review UI → 使用者修改 statement_trades
    ↓         （差異標紅、套用 A/B、快速通過）
    ↓
Confirm → statements (CONFIRMED)
    ↓
    ├─→ stock_trades（正式交易）
    └─→ user_positions（更新持倉）
```

### RAG Ingestion 流程
```
Upload/Create → files / 文字輸入
    ↓
Chunking → 文字分割
    ↓
Embedding → 呼叫 Embedding API
    ↓
Store → rag_documents + rag_chunks（含 user_id）
    ↓
Query → 向量檢索 + user_id 過濾 → citations
```
