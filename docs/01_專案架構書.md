...
8. **權證支援（v1.4）**：商品主檔標記 `asset_type=WARRANT`，權證基本資料存 `warrant_profiles`（underlying_symbol/expiry_date）；來源為 TPEx OpenAPI + TWSE ISIN fallback。
...
- `app-ocr`：OCR jobs、結果、draft trades、匯入交易（呼叫 portfolio service）。
  - **OcrService**（v1.3）：新增 `isDuplicateDraft()` 偵測重複交易，draft 回傳時自動標記 `duplicate` 欄位
