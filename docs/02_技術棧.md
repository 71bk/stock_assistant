...
- **語言 / 平台**：Java **21**（Spring Boot 4 相容）
- **框架**：Spring Boot 4.0.x（Maven Multi-Module）
- **Web**：Spring Web (REST)
- **安全**：Spring Security
...
- **商品主檔（Instrument Master）**
  - `instrument_id`（**BIGINT 自增**）作為內部主鍵（效能優、易讀、自然排序）
  - `symbol_key = {market}:{exchange}:{ticker}`（例：`US:XNAS:AAPL`、`TW:XTAI:2330`）
  - `instrument_aliases` 支援 ticker 變體（BRK.B / BRK-B）
  - **同步策略（已落地手動觸發）**：`/api/admin/instruments/sync`、`/api/admin/instruments/sync-warrants`
  - **後續擴充**：可再補 cron 定期同步與失敗重試機制
- **幣別/匯率**
  - `fx_rates` 支援 USD/TWD
...
- **Trace**：MDC `traceId`（`TraceIdFilter`）+ `X-Trace-Id` 回傳
- **Metrics**：Micrometer + Actuator `/actuator/metrics` + `/actuator/prometheus`
- **行情代理指標**：`stock.external.calls`、`stock.external.latency`、`stock.external.ratelimit.blocked`、`stock.cache.requests`、`stock.singleflight.joined`
- **外部 API 結構化欄位**：`vendor`、`endpoint`、`status`、`latency_ms`、`request_id`、`trace_id`
- **Error Tracking**：Sentry（Runtime DSN 已接；source bundle upload 由 `SENTRY_AUTH_TOKEN` 觸發）
...
- **前端 Unit**：Vitest + React Testing Library + JSDOM
- **前端 E2E**：Playwright
...
## 14) 選型總覽（TL;DR）
- **Frontend**：React 19 + Ant Design + Vite + Axios + Chart lib
- **Backend**：Spring Boot 4 (Multi-Module) + Spring Security + SSE
